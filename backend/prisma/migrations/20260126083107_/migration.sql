...
CREATE TABLE `work_configurations` (
    ...
    UNIQUE INDEX `work_configurations_department_id_key`(`department_id`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `leave_requests` (
    ...
    `status` ENUM('Pending', 'Approved', 'Rejected', 'Cancelled', 'Withdraw_Pending') NOT NULL,
...
-- AddForeignKey
ALTER TABLE `work_configurations` ADD CONSTRAINT `work_configurations_role_id_fkey` FOREIGN KEY (`role_id`) REFERENCES `roles`(`role_id`) ON DELETE SET NULL ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `work_configurations` ADD CONSTRAINT `work_configurations_department_id_fkey` FOREIGN KEY (`department_id`) REFERENCES `departments`(`department_id`) ON DELETE SET NULL ON UPDATE CASCADE;
