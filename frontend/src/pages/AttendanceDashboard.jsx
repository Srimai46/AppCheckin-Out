...
import { getAttendanceStats } from "../api/attendanceService"; // ✅ เรียกใช้ Service แทน api โดยตรง
import api from "../api/axios"; // เก็บไว้เผื่อใช้ดึง employees (HR)
import { 
  Calendar, 
  Clock, 
  User, 
...
const AttendanceCalendar = ({ year, month, stats }) => {
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    // เช็ค Priority: Absent > Leave > Late > Early > Normal
    if (stats.absentDates?.includes(dateStr)) {
        statusLabel = "Absent";
        badgeClass = "bg-rose-500 text-white shadow-rose-200 shadow-md";
    } else if (stats.leaveDates?.some(l => l.date === dateStr)) { // ✅ เพิ่มเงื่อนไข Leave (สีฟ้า)
        statusLabel = "Leave";
    ...
    days.push(
      <div key={d} className="h-24 md:h-32 border border-slate-100 rounded-2xl p-2 relative group hover:border-blue-200 transition-all bg-white">
...
export default function AttendanceDashboard() {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        setLoading(true);
        // ✅ ใช้ Function จาก Service ที่เราแก้แล้ว
  ...
  if (loading && !stats) return (
      <div className="p-10 flex justify-center text-blue-600 font-bold animate-pulse">Loading Dashboard...</div>
  );
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          ...
          <p className="text-slate-400 font-bold mt-2 flex items-center gap-2">
             <User size={16} /> 
             Viewing: <span className="text-slate-600">{stats ? stats.employeeName || "My Dashboard" : "Loading..."}</span>
        ...
        <div className="flex flex-wrap gap-3 bg-white p-2 rounded-2xl shadow-sm border border-slate-100">
           ...
           <div className="relative">
              <select 
                ...
                className="appearance-none bg-slate-50 hover:bg-slate-100 border border-transparent hover:border-slate-200 text-slate-700 font-bold py-2 pl-4 pr-10 rounded-xl cursor-pointer focus:outline-none focus:ring-2 focus:ring-blue-100 transition-all text-sm"
              >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1 space-y-6">
              ...
              <div className="bg-white p-6 rounded-[2.5rem] shadow-sm border border-slate-100 h-80">
                  ...
                  {/* ✅ เพิ่ม min-w-0 แก้ bug Recharts */}
             ...
             <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 h-full">
                <div className="flex items-center justify-between mb-6">
                    <h3 className="font-black text-slate-800 text-xl flex items-center gap-2">
                        ...
                        {month === 'All' ? 'Yearly Overview' : `${new Date(0, month-1).toLocaleString('en-US', { month: 'long' })} Calendar`}
                    </h3>
                    
                    <div className="hidden sm:flex gap-3">
                        <span className="flex items-center gap-1 text-[10px] font-bold text-rose-600 bg-rose-50 px-2 py-1 rounded-lg"><div className="w-2 h-2 rounded-full bg-rose-500"></div>Absent</span>
                        <span className="flex items-center gap-1 text-[10px] font-bold text-amber-600 bg-amber-50 px-2 py-1 rounded-lg"><div className="w-2 h-2 rounded-full bg-amber-500"></div>Late</span>
                        <span className="flex items-center gap-1 text-[10px] font-bold text-orange-600 bg-orange-50 px-2 py-1 rounded-lg"><div className="w-2 h-2 rounded-full bg-orange-500"></div>Early</span>
                        <span className="flex items-center gap-1 text-[10px] font-bold text-blue-600 bg-blue-50 px-2 py-1 rounded-lg"><div className="w-2 h-2 rounded-full bg-blue-500"></div>Leave</span>
                    </div>
                </div>
