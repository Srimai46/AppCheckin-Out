...
import { getAttendanceStats } from "../../../../src/api/attendanceService";
...
const AttendanceCalendar = ({ year, month, stats, t }) => {
  if (month === "All") {
    return (
      <div className="flex flex-col items-center justify-center h-64 text-slate-400">
        <Calendar size={48} className="mb-4 opacity-20" />
        <p className="font-bold">
          {t("attendanceDashboard.calendarHint")}
        </p>
      </div>
    );
  }
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    if (stats.absentDates?.some((a) => a === dateStr || a.startsWith(dateStr))) {
      events.push({
        label: t("attendanceDashboard.events.absent"),
    ...
    if (stats.lateDates?.includes(dateStr)) {
      events.push({
        label: t("attendanceDashboard.events.late"),
    ...
    if (stats.earlyLeaveDates?.includes(dateStr)) {
      events.push({
        label: t("attendanceDashboard.events.early"),
  ...
  return (
    <div className="grid grid-cols-7 gap-2 md:gap-4">
      {["sun", "mon", "tue", "wed", "thu", "fri", "sat"].map((day) => (
        <div
          key={day}
          className="text-center text-[10px] font-black uppercase text-slate-300 tracking-widest py-2"
        >
          {t(`attendanceDashboard.weekdays.${day}`)}
        </div>
      ))}
      {days}
    </div>
...
export default function AttendanceStats({ targetEmployeeId }) {
  ...
  const filterLabel = useMemo(
    () =>
      month === "All"
        ? t("attendanceDashboard.filterAll", { year })
        : `${pad2(month)}/${year}`,
    [month, year, t]
  );
    ...
    <div className="space-y-6">
      ...
      {loading ? (
        <div className="p-10 flex justify-center text-blue-600 font-bold animate-pulse">
          {t("attendanceDashboard.loading")}
        </div>
      ) : stats ? (
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
            <StatCard
              title={t("attendanceDashboard.stat.workingDays")}
              value={`${stats.present}/${stats.totalDaysExpected}`}
              subValue={t("attendanceDashboard.stat.presentExpected")}
              icon={Briefcase}
              bgClass="bg-blue-50/50"
              colorClass="bg-blue-500"
            />
            <StatCard
              title={t("attendanceDashboard.stat.late")}
              value={stats.late}
              subValue={`${stats.lateMinutes || 0} ${t("attendanceDashboard.stat.minutes")}`}
              icon={Clock}
              bgClass="bg-amber-50/50"
              colorClass="bg-amber-500"
            />
            <StatCard
              title={t("attendanceDashboard.stat.earlyLeave")}
              value={stats.earlyLeave}
              subValue={`${stats.earlyLeaveMinutes || 0} ${t("attendanceDashboard.stat.minutes")}`}
              icon={Timer}
              bgClass="bg-orange-50/50"
              colorClass="bg-orange-500"
            />
            <StatCard
              title={t("attendanceDashboard.stat.leaves")}
              value={stats.leave}
              subValue={t("attendanceDashboard.stat.approved")}
              icon={CheckCircle2}
              bgClass="bg-emerald-50/50"
              colorClass="bg-emerald-500"
            />
            <StatCard
              title={t("attendanceDashboard.stat.absences")}
              value={stats.absent}
              subValue={t("attendanceDashboard.stat.unexcused")}
              icon={XCircle}
...
/>
        ...
        <div className="p-8 text-center text-slate-400 font-bold border-2 border-dashed border-slate-100 rounded-3xl">
          {t("attendanceDashboard.noDataFound")}
        </div>
      )}
    </div>
  );
