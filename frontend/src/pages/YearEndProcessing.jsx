...
const YearEndProcessing = () => {
  ...
  const buildProcessConfirmHtml = () => {
    const items = [
      ...
      `Data for <b>${escapeHtml(
        lastYear
      )}</b> will be "<b>locked</b>" immediately.`,
    ];
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      await alertSuccess(
        ...
        res?.message ||
          "Year-end processing and quota assignment completed successfully."
      );
      await fetchConfigs();
    } catch (error) {
      const errorMsg =
  ...
  const FUTURE_YEARS = 3; // เลือกได้ล่วงหน้า 10 ปี
        ...
        <div className="max-w-5xl mx-auto">
          <header className="mb-8">
            <h1 className="text-2xl font-bold text-gray-800 flex items-center gap-2">
              <Calendar className="text-indigo-600" /> Year-End Processing &
              Quota Assignment
            </h1>
            <p className="text-gray-500">
              Carry over leave balances and assign new yearly quotas in one
              step.
            </p>
          </header>
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-gray-500 mb-6">
                  Set the base leave quotas to be assigned to all employees
                  along with carry-over.
                </p>
                  ...
                  {Object.keys(quotas).map((type) => (
                    <div key={type}>
                      ...
                      <input
                        ...
                        value={quotas[type]}
                        onChange={(e) =>
                          handleQuotaChange(type, e.target.value)
                        }
      ...
      `}
          ...
          <div className="mt-6 flex items-center gap-3 text-amber-700 bg-amber-50 p-4 rounded-3xl border border-amber-100">
            ...
            <div className="text-xs font-bold leading-relaxed uppercase tracking-tight">
              Warning: Pressing this button will "overwrite" the quotas for all
              employees in the target year. And immediately lock last year's
              data. Please ensure the correct number of leave days to be
              distributed.
            </div>
          </div>
        </div>
